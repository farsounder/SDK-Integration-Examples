...
#include "autogenerated/nav_api.pb.h"


const int kLargeMessageBytesLimit = 128 << 20;  // 128MB
const int kLargeMessageBytesWarningThreshold = 110 << 20;  // 110MB
// Using 'localhost' doesn't work on WSL2
const std::string kHost = "192.168.160.1";
...
void send_generic_request(
  ...
  printf("khost = %s\n", kHost.c_str());
  zmq_socket.connect("tcp://" + kHost + ":" + port);
  ...
  zmq::recv_result_t r = zmq_socket.recv(update, zmq::recv_flags::none);
...
void request_reply_example()
{
  ...
  printf("Check for response\n");
  zmq::recv_result_t res = req_processorSettings.recv(
    update, zmq::recv_flags::none);
  // zmq response back to a proto message
...
int main()
{
    ...
    */
    ...
    while (true) {
      // Bottom detection
      printf("Press ENTER to send request to change bottom to: %d\n", bottom_detect);
      getchar();
      toggle_bottom_detection(bottom_detect);
      printf("\n");

      // Auto squelch
      printf("Press ENTER to send request to change autosquelch to: %d\n",
        auto_squelch_on);
      getchar();
      ...
      if (!auto_squelch_on) {
        ...
        printf("Press ENTER to send request to manual squelch to: %f\n",
          squelch_value);
        getchar();
      ...
      printf("Press ENTER to change field of view / processor mode to: %d\n",
